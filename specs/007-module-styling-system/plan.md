...
**Branch**: `007-module-styling-system` | **Date**: 2025-07-17 | **Spec**: `specs/007-module-styling-system/spec.md`
...
2. A **preset browser** showing 5 system presets and user-saved presets as color-swatch thumbnail cards, with user presets sorted newest first and one-click apply guarded by a confirmation when unsaved editor changes would be replaced.
...
|-----------|--------|-------|
| I. Folder Structure & Module Boundaries | ✅ PASS | Feature code in `src/features/styling/`, API in `src/api/`, types in `src/lib/types/`. No cross-feature imports. |
...
| XII. Testing | ✅ PASS | Zod schema unit tests, hook tests with MSW mocks, component interaction tests with RTL. |
...
src/
├── api/
│   ├── notebooks.ts         # + updateNotebookStyles (bulk PUT)
│   └── presets.ts            # + renameUserPreset, applyPreset
├── features/
│   └── styling/
│       ├── components/
...
│           ├── style-defaults.ts         # Default style values, swatch palette
...
**Structure Decision**: Single-project React SPA. Feature code grouped under `src/features/styling/` per constitution Principle I. Types already exist in `src/lib/types/styles.ts` and `src/lib/types/notebooks.ts`. API functions extend existing `src/api/notebooks.ts` and `src/api/presets.ts`. The UI remains desktop-only in this feature and relies on a right-side Sheet drawer with a horizontally scrollable tab strip.
